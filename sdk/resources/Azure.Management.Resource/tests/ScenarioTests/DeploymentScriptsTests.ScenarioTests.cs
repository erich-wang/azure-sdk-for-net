...
namespace DeploymentScripts.Tests
{
    ...
    {
        public LiveDeploymentScriptsTests(bool isAsync, ResourceManagementClientOptions.ServiceVersion serviceVersion)
        ...
        public async Task CanCrudSimpleDeploymentScript()
        {
            ...
            var userAssignedIdentities = new Dictionary<string, UserAssignedIdentity>
                {
                    {
                        $"/subscriptions/{subscriptionId}/resourceGroups/{ResourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/filiz-user-assigned-msi",
        ...
        public async Task CanReturnErrorOnScriptExecutionFailure()
        {
            ...
            var rawcreateDeploymentScriptResult = await DeploymentScriptsClient.StartCreateAsync(ResourceGroupName, deploymentScriptName, deploymentScript) ;
            var rcdResult = (await DeploymentScriptsClient.GetAsync(ResourceGroupName, deploymentScriptName)).Value as AzurePowerShellScript;
            //var createDeploymentScriptResult = rawcreateDeploymentScriptResult.Value as AzurePowerShellScript;
            Assert.NotNull(rcdResult);
            Assert.AreEqual(ScriptProvisioningState.Creating, rcdResult.ProvisioningState);
