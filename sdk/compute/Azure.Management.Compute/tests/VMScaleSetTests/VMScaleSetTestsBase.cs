...
namespace Azure.Management.Compute.Tests
{
    public class VMScaleSetTestsBase :VMTestBase
    {
                        ...
                        NetworkInterfaceConfigurations = new List<VirtualMachineScaleSetNetworkConfiguration>()
                        {
                            new VirtualMachineScaleSetNetworkConfiguration(Recording.GenerateAssetName("vmsstestnetconfig"))
                            {
                                ...
                                IpConfigurations = new List<VirtualMachineScaleSetIPConfiguration>
                                {
                                    ...
                                    {
                                        Subnet = new Azure.Management.Compute.Models.ApiEntityReference()
                                        {
                                            Id = subnetId
                                        },
                                        LoadBalancerBackendAddressPools = (loadBalancerBackendPoolId != null) ?
                                            new List<Azure.Management.Compute.Models.SubResource> {
                                                new Azure.Management.Compute.Models.SubResource(loadBalancerBackendPoolId)
                                            } : null,
                                        ApplicationGatewayBackendAddressPools = new List<Azure.Management.Compute.Models.SubResource>(),
                                    }
                                }
                            }
                        }
        ...
        protected async void UpdateVMScaleSet(string rgName, string vmssName, VirtualMachineScaleSet inputVMScaleSet)
        {
            var createOrUpdateResponse = await VirtualMachineScaleSetsClient.StartCreateOrUpdateAsync(rgName, vmssName, inputVMScaleSet);
        }
            ...
            AutomaticRepairsPolicy automaticRepairsPolicy = null)
        {
            ...
            if (ppgId != null)
            {
                inputVMScaleSet.ProximityPlacementGroup = new Azure.Management.Compute.Models.SubResource() { Id = ppgId };
            }
