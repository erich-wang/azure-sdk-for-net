...
namespace Azure.Management.Compute.Tests
{
    public class GalleryTests : VMTestBase
    {
        ...
        public async Task Gallery_CRUD_Tests()
        {
            ...
            await GalleriesClient.StartDeleteAsync(rgName, galleryName);
            await GalleriesClient.StartDeleteAsync(rgName2, galleryName2);
    ...
    //galleryImageVersionName, ReplicationStatusTypes.ReplicationStatus);
                ...
                await GalleryImageVersionsClient.StartDeleteAsync(rgName, galleryName, galleryImageName, galleryImageVersionName);
                ...
                await ImagesClient.StartDeleteAsync(rgName, imageName);
                Trace.TraceInformation("Deleted the CRP image.");
                await VirtualMachinesClient.StartDeleteAsync(rgName, vm.Name);
                Trace.TraceInformation("Deleted the virtual machine.");
                await GalleryImagesClient.StartDeleteAsync(rgName, galleryName, galleryImageName);
        ...
        public async Task GalleryApplication_CRUD_Tests()
        {
            ...
            await GalleryApplicationsClient.StartDeleteAsync(rgName, galleryName, galleryApplicationName);
        ...
        public async Task GalleryApplicationVersion_CRUD_Tests()
        {
            ...
            try
            {
                ...
                await GalleryApplicationVersionsClient.StartDeleteAsync(rgName, galleryName, galleryApplicationName, galleryApplicationVersionName);
        ...
        private async Task<string> CreateApplicationMediaLink(string rgName, string fileName)
        {
            ...
            if (Mode == RecordedTestMode.Record)
            {
                ...
                //BlobContainer container = BlobContainersClient.GetContainerReference("sascontainer");
                //bool created = container.CreateIfNotExistsAsync().Result;

                //var PageBlob pageBlob = container.GetPageBlobReference(fileName);
        ...
        {
            return new GalleryApplication(computeManagementTestUtilities.DefaultLocation)
            {
